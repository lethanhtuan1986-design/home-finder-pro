...
const SearchPage = () => {
  ...
  const advertisements = useMemo(() => infiniteData?.pages.flatMap((p: any) => p?.items || []) ?? [], [infiniteData]);
          ...
          <div className="flex gap-6">
            <div className={showMap ? "w-full lg:w-3/5" : "w-full"}>
              {loading && advertisements.length === 0 && (
                <div
                  className={`grid ${showMap ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"} gap-6`}
                >
                ...
                <div className="sticky top-20 h-[calc(100vh-8rem)]">
                  <MapView
                    locations={advertisements.map((ad: any) => ({
                      point: ad.apartmentUu?.point
                        ? String(ad.apartmentUu.point)
                        : `[${ad.apartmentUu?.latitude || 0},${ad.apartmentUu?.longitude || 0}]`,
